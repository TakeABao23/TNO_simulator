...
class Population():
    def __init__(self, reference_pop, init_params=None, detection_prob = None):
        ...
        self.wide = InitWideParams().wide_params
        self.ref_binary_frac = self.moonlike['fb'] + self.wide['fb']

        # Orbit parameters (probability distribution)
        self.morb = OrbitParamDist(self.moonlike)
        self.worb = self.morb
    ...
    def population_simulator(self, reference_pop=None):
        ...
        for _, row in reference_pop.iterrows():
            ...
            if binary_check > (self.ref_binary_frac):
                ...
                orb = self.morb
                orbital_params = [orb.a, orb.e, orb.i, orb.w, orb.Om, orb.mu]
